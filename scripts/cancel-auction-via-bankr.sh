...
usage() {
  cat <<USAGE
Usage:
  ./scripts/cancel-auction-via-bankr.sh --token-id <id> [--contract-mode ownership-given|own-deployed] [--contract <address>] [--deploy-receipt <path>] [--chain mainnet|sepolia|base|base-sepolia] [--broadcast] [--note <text>]
USAGE
}
...
CALLDATA="$(cast calldata 'cancelAuction(address,uint256)' "$COLLECTION_CONTRACT" "$TOKEN_ID")"
DESCRIPTION_BASE="${NOTE_OVERRIDE:-Rare SynETHsis auction cancel via aaigotchi}"

printf 'SuperRare cancel preview\n'
printf '  Chain: %s (%s)\n' "$CHAIN" "$CHAIN_ID"
...
RECEIPT_PATH="$PROJECT_RECEIPTS_DIR/${STAMP_UTC//:/-}-rare-synethsis-auction-cancel.json"
RECEIPT_PAYLOAD="$(jq -n \
  --arg schema 'aaigotchi.rare-synethsis.auction-cancel.receipt.v1' \
...
echo
echo 'SuperRare auction cancelled via Bankr'
